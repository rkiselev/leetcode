...
public class MergeTripletsToFormTargetTriplet {
     ...
     * - Choose the first and last triplets [[2,5,3],[1,8,4],[1,7,5]]. Update the last triplet to be [max(2,1),
     * max(5,7), max(3,5)] = [2,7,5]. triplets = [[2,5,3],[1,8,4],[2,7,5]]
    ...
    public static void main(String[] args) {
        assertTrue(new Solution().mergeTriplets(new int[][]{{4,5,2},{4,2,7},{5,8,6},{3,6,6},{4,5,2}}, new int[]{4,5,7}));
        new Solution().mergeTriplets(new int[][]{{2,5,3},{1,8,4},{1,7,5}}, new int[]{2,5,7});
    }
        ...
        public boolean mergeTriplets(int[][] triplets, int[] target) {
            ...
            for (int i = 0; i < triplets.length; i++) {
                ...
                for (int j = 0; j < triplets[i].length; j++) {
                    if (triplets[i][j] == target[j]) good.add(j);
                }
            }
            return good.size() == 3;
        }
